...
如果找到多个相同类型的组件，再用属性作为bean的id去查容器

如果我们想使用名称装配可以结合@Qualifier注解进行使用，而不用属性名

@primary让spring装配时优先选择此bean，但优先级不如@Qualifier指定的bean

若还是无法匹配上则会抛出 BeanCreationException 异常
