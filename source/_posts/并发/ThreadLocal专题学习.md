...
![20210522143326](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522143326.png)

`Thread` 类有一个类型为 `ThreadLocal`.`ThreadLocalMap`的实例变量`threadLocals`，也就是说每个线程有一个自己的`ThreadLocalMap`。

`ThreadLocalMap`有自己的独立实现，可以简单地将它的key视作`ThreadLocal`，`value`为代码中放入的值（实际上`key`并不是`ThreadLocal`本身，而是它的一个**弱引用**）。
...
![20210522143858](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522143858.png)
...
所以这里在GC之后，key就会被回收，我们看到上面`debug`中的`referent=null`, 如果**改动一下代码**：

![20210522144000](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522144000.png)

这个问题刚开始看，如果没有过多思考，**弱引用**，还有**垃圾回收**，那么肯定会觉得是`null`。

其实是不对的，因为题目说的是在做 `ThreadLocal.get()` 操作，证明其实还是有强引用存在的，所以 `key` 并不为 `null`，如下图所示，`ThreadLocal`的**强引用**仍然是存在的。

![20210522144111](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522144111.png)

如果我们的**强引用**不存在的话，那么 `key` 就会被回收，也就是会出现我们 `value` 没被回收，`key` 被回收，导致 `value` 永远存在，出现内存泄漏。

### ThreadLocal.set()方法源码详解

![20210522144201](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522144201.png)
...
![20210522144507](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522144507.png)
...
![20210522144732](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522144732.png)
...
![20210522145034](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145034.png)

这里直接将数据放到该槽位即可。

**第二种情况：** 槽位数据不为空，`key`值与当前`ThreadLocal`通过`hash`计算获取的`key`值一致：

![20210522145114](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145114.png)

这里直接更新该槽位的数据。

**第三种情况：** 槽位数据不为空，往后遍历过程中，在找到`Entry`为`null`的槽位之前，没有遇到`key`过期的`Entry`：

![20210522145153](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145153.png)
...
![20210522145307](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145307.png)

散列数组下标为7位置对应的`Entry`数据`key`为`null`，表明此数据key值已经被垃圾回收掉了，此时就会执行`replaceStaleEntry()`方法，该方法含义是**替换过期数据的逻辑**，以**index=7**位起点开始遍历，进行探测式数据清理工作。
...
![20210522145442](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145442.png)
...
![20210522145551](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145551.png)
...
![20210522145645](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145645.png)

**向后遍历过程中，如果没有找到相同key值的Entry数据：**

![20210522145703](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145703.png)
...
![20210522145757](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522145757.png)
...
![20210522150104](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210522150104.png)
...
10. 2 `rehash()`中会先进行一轮探测式清理，清理过期`key`，清理完成后如果size >= threshold - threshold / 4，就会执行真正的扩容逻辑(扩容逻辑往后看)
...
![20210523103810](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523103810.png)
...
![20210523104043](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523104043.png)
...
![20210523104730](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523104730.png)

我们假设`expungeStaleEntry(3)` 来调用此方法，如上图所示，我们可以看到`ThreadLocalMap`中`table`的数据情况，接着执行清理操作：

![20210523104808](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523104808.png)

第一步是清空当前`staleSlot`位置的数据，`index=3`位置的`Entry`变成了`null`。然后接着往后探测：

![20210523104839](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523104839.png)

执行完第二步后，index=4的元素挪到index=3的槽位中。

继续往后迭代检查，碰到正常数据，计算该数据位置是否偏移，如果被偏移，则重新计算`slot`位置，目的是让正常数据尽可能存放在正确位置或离正确位置更近的位置

![20210523104901](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523104901.png)
...
![20210523122555](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523122555.png)

接着看看具体的`resize()`方法，为了方便演示，我们以`oldTab.len=8`来举例：

![20210523122623](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523122623.png)
...
![20210523122852](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523122852.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![20210523122923](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523122923.png)
...
![20210523123134](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523123134.png)
...
![20210523123400](http://marlowe.oss-cn-beijing.aliyuncs.com/img/20210523123400.png)
...
[ThreadLocal 关键字解析](https://snailclimb.gitee.io/javaguide/#/docs/java/multi-thread/%E4%B8%87%E5%AD%97%E8%AF%A6%E8%A7%A3ThreadLocal%E5%85%B3%E9%94%AE%E5%AD%97)
